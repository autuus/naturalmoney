...
class account{
	...
	function validate_payment($account_id, $account_owner){
		if (!$account_id) {
			throw new Exception("Syِtن tilinumero");
		}
		if (!$account_owner) {
			throw new Exception("Syِtن saajan nimi");
		}
		if (!$to_account = $this->recursion->database->account->select("id=$account_id")) {
			throw new Exception("Kنyttنjن tiliن ei lِytynyt");
		}
		if (!$to_person = $this->recursion->database->person->select("name='$account_owner'")) {
			throw new Exception("Henkilِن ei lِytynyt");
		}
		if ($to_person["id"] != $to_account["owner"]) {
			throw new Exception("Tili ja henkilِ ei tنsmنن");
		}
		return true;
	}
